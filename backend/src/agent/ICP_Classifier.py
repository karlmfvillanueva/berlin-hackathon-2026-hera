...
and returns booking-psychology ICP classification as structured JSON via Claude tool use.
"""
...
_TOOL_SCHEMA: dict[str, Any] = {
    "name": "icp_classifier",
    "description": (
        "Booking ICP classification: best guest persona for conversion, secondaries, rejects, "
        "and conversion summary. Persona strings must match the allowed list exactly."
    ),
    "input_schema": {
        "type": "object",
        "properties": {
...
def classify_icp(scrape: Mapping[str, Any]) -> dict[str, Any]:
    ...
    response = client.messages.create(
        ...
        tool_choice={"type": "tool", "name": "icp_classifier"},
        messages=[{"role": "user", "content": _build_user_message(scrape)}],
    )

    tool_block = next((b for b in response.content if b.type == "tool_use"), None)
    if not tool_block:
        raise RuntimeError("ICP classifier returned no tool_use block")
