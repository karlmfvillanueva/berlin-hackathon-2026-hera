...
and returns booking-conversion-focused location insight as structured JSON via Claude tool use.
"""
...
_TOOL_SCHEMA: dict[str, Any] = {
    ...
    "description": (
        "Booking-conversion location analysis for video creative. "
        "Every string must tie location to guest outcomes; ban vague centrality clichés."
    ),
    "input_schema": {
        "type": "object",
        "properties": {
            ...
            "walkability_value": {
                ...
                "required": ["daily_rhythm_without_car", "planning_friction_removed", "concrete_examples"],
            },
            ...
            "creative_translation": {
                ...
                "required": ["on_screen_hook_ideas", "b_roll_or_map_direction", "emotional_carrier_line"],
            },
        },
        "required": [
            "location_summary",
...
def enrich_location(scrape: Mapping[str, Any]) -> dict[str, Any]:
    ...
    response = client.messages.create(
        ...
        tool_choice={"type": "tool", "name": "location_enrichment"},
        messages=[{"role": "user", "content": _build_user_message(scrape)}],
    )

    tool_block = next((b for b in response.content if b.type == "tool_use"), None)
    if not tool_block:
        raise RuntimeError("Location enrichment returned no tool_use block")
    ...
    if not isinstance(result.get("landmark_proximity"), list) or len(result["landmark_proximity"]) < 2:
        raise RuntimeError("landmark_proximity must be a list with at least 2 items")
    if not isinstance(result.get("best_trip_occasions"), list) or len(result["best_trip_occasions"]) < 2:
