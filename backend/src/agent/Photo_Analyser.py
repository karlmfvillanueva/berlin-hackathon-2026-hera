...
from ICP, Location, and Reviews agents. Uses vision on hosted image URLs where
possible, scores every asset for conversion video use, rejects weak shots, and
returns exactly five hero-first indices (or fewer if the listing has fewer than
five photos) for Final Assembly + Hera reference URLs.
"""
...
Output only via the provided tool — strict structured fields, no preamble."""


_TOOL_SCHEMA: dict[str, Any] = {
    "name": "photo_analyser",
    "description": (
        "Vision- and strategy-backed selection of up to five reference listing photos "
        "(hero-first indices) plus per-photo scores, rejects, and narrative roles."
    ),
    "input_schema": {
        "type": "object",
        "properties": {
...
def _validate_result(result: dict[str, Any], n_photos: int) -> None:
    ...
    for i, row in enumerate(rej):
        ...
        for k in ("reason", "what_it_would_wrongly_signal"):
            if not isinstance(row.get(k), str) or not str(row[k]).strip():
                raise RuntimeError(f"rejected_gallery_strengths[{i}].{k} must be a non-empty string")
    ...
    if not isinstance(slots, list) or len(slots) != len(normalized):
        raise RuntimeError("narrative_slot_plan length must match selected_indices_hero_first")
    allowed_slots = frozenset(
        {"hook", "context", "core_experience", "payoff", "proof_or_climax"}
    )
    ...
    if sorted(slot_indices) != sorted(normalized):
        raise RuntimeError("narrative_slot_plan must reference each selected index exactly once")

    notes = result.get("creative_director_notes_for_assembly")
    if not isinstance(notes, str) or len(notes.strip()) < 40:
        raise RuntimeError("creative_director_notes_for_assembly must be substantive (min ~40 chars)")
...
def analyse_photos(
    ...
    n = len(catalog)
    if n == 0:
        raise RuntimeError("Photo analyser requires presentation.photo_assets with at least one URL")
    ...
    response = client.messages.create(
        ...
        tool_choice={"type": "tool", "name": "photo_analyser"},
        messages=[{"role": "user", "content": _build_message_content(
            catalog,
            icp,
            location_enrichment,
            reviews_evaluation,
    ...
    if not tool_block:
        raise RuntimeError("Photo analyser returned no tool_use block")
